// ══════════════════════════════════════════════════════════════════════════════
// FirestoreService — LIVE VERSION (Multi-Tenant with userId filtering)
// ══════════════════════════════════════════════════════════════════════════════
...
class FirestoreService {
  ...
  Future<AppUser> loginUser({required String email, required String password}) async {
    final q = await _users.where('email', isEqualTo: email).limit(1).get();
    if (q.docs.isEmpty) throw Exception('No account found for this email.');
    final data = _fromSnap(q.docs.first);
    final storedPassword = data['password'] as String? ?? '';
    if (storedPassword != password) throw Exception('Incorrect password.');
  ...
  Future<AppUser> registerUser({required String email, required String name, required String password}) async {
    ...
    data.remove('id');
    data['password'] = password; // Store password in Firestore
    await ref.set(data);
    return user;
  }

  // ── Trucks (userId-scoped) ────────────────────────────────────────────────
  Stream<List<Truck>> watchTrucks(String userId) =>
      _trucks.where('userId', isEqualTo: userId).orderBy('vehicleNo').snapshots()
  ...
  // ── Drivers (userId-scoped) ───────────────────────────────────────────────
  Stream<List<Driver>> watchDrivers(String userId) =>
      _drivers.where('userId', isEqualTo: userId).orderBy('name').snapshots()
  ...
  // ── Routes (userId-scoped) ────────────────────────────────────────────────
  Stream<List<FreightRoute>> watchRoutes(String userId) =>
      _routes.where('userId', isEqualTo: userId).where('isActive', isEqualTo: true).orderBy('origin').snapshots()
  ...
  // ── Bilties (userId-scoped) ───────────────────────────────────────────────
  Stream<List<Bilty>> watchBilties(String userId) =>
      _bilties.where('userId', isEqualTo: userId).orderBy('createdAt', descending: true).snapshots()
  ...
  // ── Challans (userId-scoped) ──────────────────────────────────────────────
  Stream<List<Challan>> watchChallans(String userId) =>
      _challans.where('userId', isEqualTo: userId).orderBy('createdAt', descending: true).snapshots()
  ...
  Stream<List<Trip>> watchTrips(String userId) =>
      _trips.where('userId', isEqualTo: userId).orderBy('createdAt', descending: true).snapshots()
