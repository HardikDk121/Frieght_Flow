  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const FreightAppBar(
          title: 'Trip Management', subtitle: 'Track and update trip status'),
...
class TripDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final vm = context.watch<TripManagementProvider>();
    final trip = vm.trips
        .firstWhere((t) => t.id == tripId, orElse: () => vm.trips.first);
    final route = vm.routeForTrip(trip);
    final truck = vm.truckForTrip(trip);
    final driver = vm.driverForTrip(trip);
    ...
    return Scaffold(
      appBar:
          FreightAppBar(title: trip.tripNo, subtitle: route?.displayName ?? ''),
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          // State machine card
          ...
          _infoGrid([
            ...
            (
              'Capacity',
              truck != null
                  ? CurrencyFormatter.formatWeight(truck.capacityKg)
                  : '—',
              Icons.scale_outlined
            ),
            (
              'Scheduled',
              DateFormat('dd MMM yyyy').format(trip.scheduledDate),
              Icons.calendar_today_outlined
            ),
            if (trip.actualDispatchDate != null)
              (
                'Dispatched',
                DateFormat('dd MMM').format(trip.actualDispatchDate!),
                Icons.departure_board_outlined
              ),
            if (trip.actualDeliveryDate != null)
              (
                'Delivered',
                DateFormat('dd MMM').format(trip.actualDeliveryDate!),
        ...
        if (!isTerminal) ...[
            ...
            child:
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: (_selectedNext == null || vm.isLoading)
                      ? null
                      : () => _advance(context, vm),
                  icon: vm.isLoading
                      ? const SizedBox(
                  ...
                  label: Text(
                      vm.isLoading ? 'Updating...' : 'Confirm Status Update'),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: _selectedNext == TripState.cancelled
                        ? AppColors.danger
                        : AppColors.primary,
                  ),
                ),
          ...
          Padding(
            ...
            child: Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                color: widget.trip.state == TripState.delivered
              ...
              child: Row(children: [
                Icon(
                  widget.trip.state == TripState.delivered
                      ? Icons.check_circle_outline
                ...
                const SizedBox(width: 8),
                Text(
                    'Trip ${TripStateHelper.label(widget.trip.state)}. No further updates possible.',
                    style: TextStyle(
                        fontSize: 13,
                        color: widget.trip.state == TripState.delivered
                            ? AppColors.success
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Row(children: [
          ...
          const SizedBox(width: 12),
          Expanded(
              child: Text(TripStateHelper.label(s),
                  style: TextStyle(
                      fontSize: 14,
                      fontWeight: FontWeight.w500,
                      color: selected ? color : AppColors.textPrimary))),
          Icon(isCancel ? Icons.cancel_outlined : Icons.arrow_forward_rounded,
      ...
      return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(
          ...
          child:
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(
                '${TripStateHelper.label(e.from)}  →  ${TripStateHelper.label(e.to)}',
                style: const TextStyle(
                    fontSize: 13,
                    fontWeight: FontWeight.w600,
                    color: AppColors.textPrimary)),
        ...
        Expanded(
          ...
          Text(bilty.biltyNo,
              style: const TextStyle(
                  ...
                  color: AppColors.primary)),
          Text(
              '${bilty.consignorName} → ${bilty.consigneeCity}  ·  ${CurrencyFormatter.formatWeight(bilty.weightKg)}',
              style: const TextStyle(
...
class _StatePill extends StatelessWidget {
  ...
  Color get _bg {
    switch (state) {
      case TripState.godown:
        return AppColors.warningLight;
      case TripState.dispatched:
        return AppColors.infoLight;
      case TripState.inTransit:
        return AppColors.infoLight;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(TripStateHelper.label(state),
          style:
              TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: _fg)),
    );
  }
}
    ...
    return Row(
      ...
      if (i.isOdd) {
        return Expanded(
            child: Container(
                height: 2,
                color: (i ~/ 2) < idx ? AppColors.primary : AppColors.divider));
      }
      final si = i ~/ 2;
      final done = si < idx;
      final active = si == idx;
      return Column(children: [
        ...
        const SizedBox(height: 3),
        Text(TripStateHelper.label(_steps[si]),
            style: TextStyle(
                fontSize: 9,
...
class _NewTripScreenState extends State<NewTripScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const FreightAppBar(
          title: 'New Trip', subtitle: 'Assign challan, truck & driver'),
      body: Form(
        key: _formKey,
        child: ListView(
          padding: const EdgeInsets.all(16),
          children: [
            ...
            if (openChallans.isEmpty)
              _noItemsBanner(
                  'No open challans available. Create a challan first.',
                  '/challan')
            else
              _dropdown(
                ...
                items: openChallans
                    ...
                    .map((c) => DropdownMenuItem(
                        value: c.id,
                        child: Text(
                            '${c.challanNo}  (${c.totalBilties} bilties)')))
                    .toList(),
            ...
            if (availTrucks.isEmpty)
              _noItemsBanner(
                  'No trucks available. Check master data.', '/master')
            else
              _dropdown(
                label: 'Truck *',
                value: _truckId,
                items: availTrucks
                    .map((t) => DropdownMenuItem(
                        value: t.id,
                        child: Text(
                            '${t.vehicleNo}  –  ${t.make} ${t.model}  (${CurrencyFormatter.formatWeight(t.capacityKg)})')))
                    .toList(),
  ...
  Widget _challanPreview(BuildContext context, String cId) {
    ...
    final bilties =
        context.read<ChallanProvider>().getBiltiesForChallan(challan);
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 8),
        Text(
            '${bilties.length} bilties  ·  ${CurrencyFormatter.formatWeight(totalW)}',
            style: const TextStyle(
                fontSize: 13,
